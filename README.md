...
[Live demo on github pages](https://www.takuzen.me/hugo-theme-codebox/).

Some works are still in progress. See [TODOS](#todos) below.

## Install

1. clone codebox to your hugo site's `themes` folder.
```
git clone https://github.com/tywhisky/hugo-theme-codebox.git themes/codebox
```

2. change your theme to codebox in your site config
```toml
# config.toml
...
[params]
  ...
  projectsUrl = "https://github.com/tywhisky"
```
...
for former hexo codebox users: please assign your json array to a `list` key.
...
[[params.social]]
  name = "email"
  link = "xxx@gmail.com" # no need for "mailto:" at the start
...
Assign your copy right to `.Site.Copyright`. codebox will append current year to the head.

TODO: Customizable copyright year

```toml
copyright = "Zeran Wu" # codebox theme will use site title if copyright is not set
```
...
The default engine is disqus. **By now only disqus is supported in codebox.** I will add more options sooner or later. See [Comments Alternatives](https://gohugo.io/content-management/comments/#comments-alternatives)

Before using disqus, you need to register and get your [disqus shortname](https://help.disqus.com/en/articles/1717111-what-s-a-shortname). Assign your shortname in `.Site.disqusShortname`, or codebox will use `.Site.Title` by default.

```
disqusShortname = "wzr" # codebox will use site title if not set
```
...
codebox uses hugo's bulit in analytics templates. Check [hugo's documents](https://gohugo.io/templates/internal#google-analytics) for details.
...
codebox supports mathjax. Just add `mathjax` option in your site config:
```toml
[params]
